...
def save_log(run_name, actions, states, losses, i):
...
if __name__ == '__main__':
    ...
    for i in range(episodes):
        ...
        for b in range(nSteps):
            ...
            if (seq_label[0]==1000):
                break
            seq_data = torch.tensor(seq_data, dtype=torch.float32).to(dev)
            seq_label = torch.tensor(seq_label, dtype=torch.float32).to(dev)
            optimizer.zero_grad()
            # Prediction
        ...
        save_log(run_name, actions, states, losses, i)
